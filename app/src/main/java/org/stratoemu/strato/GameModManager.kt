...
class GameModManager(
    ...
    private fun installDiscoveredPackages(sourceRoot: File, fallbackName: String): Int {
        ...
        if (packages.isEmpty())
            throw IllegalArgumentException("Package does not contain supported ExeFS, RomFS, or cheats for this game")
    ...
    private fun installPackage(source: File, packageName: String) {
        ...
        try {
            ...
            if (!isModPackage(staging))
                throw IllegalArgumentException("Package does not contain supported ExeFS, RomFS, or cheats")
