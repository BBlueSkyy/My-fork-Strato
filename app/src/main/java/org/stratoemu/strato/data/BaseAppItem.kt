...
private val missingIcon by lazy { ContextCompat.getDrawable(StratoApplication.instance, R.drawable.default_icon)!!.toBitmap(256, 256) }
...
open class BaseAppItem(private val meta : AppEntry, val enabled: Boolean = false) : Serializable {
    ...
    fun loaderResultString(context : Context) = context.getString(
        when (meta.loaderResult) {
            ...
            LoaderResult.ErrorSparseNCA -> R.string.sparse_nca_error

            LoaderResult.ErrorCompressedNCA -> R.string.compressed_nca_error
        }
    )

    /**
     * Get the display version - shows update version if available, otherwise base version
     */
    fun getDisplayVersion(context: Context): String {
        titleId?.let { titleId ->
            val enabledUpdate = org.stratoemu.strato.ContentManager.getEnabledUpdate(titleId)
            if (enabledUpdate != null) {
