    ...
    namespace memory {
        ...
        union MemoryAttribute {
            ...
            struct {
                ...
                bool isPermissionLocked : 1; //!< Set via svcSetMemoryAttribute; one-way (can be set but never
                                              //!< cleared through that SVC) and blocks further permission changes
            };
            u8 value;
        };
    ...
    namespace kernel {
        ...
        class MemoryManager {
          ...
          public:
            ...
            /**
             * @brief Atomically validates and sets `attributes.isPermissionLocked` for chunks within a range
             * @return False if any chunk within the range has `attributeChangeAllowed` set to false, in which
             * case nothing in the range is modified; true if the lock was applied to the entire range
             * @note This is one-way from the SVC's perspective - svcSetMemoryAttribute can only set this bit,
             * never clear it - so there is no `value` parameter, unlike `SetRegionCpuCachingIfAllowed`
             */
            bool SetRegionPermissionLockedIfAllowed(span<u8> memory);
