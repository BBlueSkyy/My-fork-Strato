...
namespace skyline::nce {
    ...
    void NCE::SvcHandler(u16 svcId, ThreadContext *ctx, u64 guestSp, u64 guestFp) {
        ...
        if (svcId == 0x26) {
            ...
            auto readGuest = [&](u64 address, void *output, size_t size) {
                if (!address)
                    return false;

                auto *guestAddress{reinterpret_cast<u8 *>(address)};
                span<u8> guestRange{guestAddress, size};
                if (!state.process->memory.AddressSpaceContains(guestRange))
                    return false;

                auto chunk{state.process->memory.GetChunk(guestAddress)};
                if (!chunk || !chunk->second.permission.r ||
                    guestAddress + size > chunk->first + chunk->second.size)
                    return false;

                auto hostRange{state.process->memory.GetHostSpan(guestRange)};
                std::memcpy(output, hostRange.data(), size);
                return true;
            };

            // The per-SVC trampoline stores the address immediately after the
            // original guest SVC at the first word of this temporary frame.
            u64 originalGuestLr{};
            if (readGuest(guestSp, &originalGuestLr, sizeof(originalGuestLr)) && originalGuestLr)
                guestFrames.push_back(reinterpret_cast<void *>(originalGuestLr));

            // Walk only valid, forward-moving AArch64 frame records. Reading
            // through MemoryManager avoids a host crash on a malformed frame.
            u64 framePointer{guestFp};
            for (u32 depth{}; depth < 64 && framePointer; depth++) {
                if (framePointer & 0xF)
                    break;

                signal::StackFrame frame{};
                if (!readGuest(framePointer, &frame, sizeof(frame)))
                    break;
                ...
                if (next <= framePointer || next - framePointer > 0x100000)
                    break;
                framePointer = next;
            }

            if (!guestFrames.empty())
                LOGE("SOLATERIA-BREAK-STACK:{}", state.loader->GetStackTrace(guestFrames));
            else
                LOGE("SOLATERIA-BREAK-STACK: unavailable (SP=0x{:X}, FP=0x{:X})", guestSp, guestFp);
        }
    ...
    u32 *WriteTrampoline(u32 *code, u64 target) {
        ...
        *code++ = 0x910003E2; // MOV X2, SP -- third argument: guest SP
        *code++ = 0xF9415423; // LDR X3, [X1, #0x2A8] (ThreadContext::hostSp)
        *code++ = 0x9100007F; // MOV SP, X3
        *code++ = instructions::Mov(registers::X3, registers::X29).raw; // fourth argument: guest FP

        /* Store Skyline TLS + guest SP on stack */
        *code++ = 0xA9BF0BE1; // STP X1, X2, [SP, #-16]!

        /* Jump to the handler while preserving X2/X3 as guest SP/FP arguments */
