...
namespace skyline::jit {
    ...
    CallbackOrAccessOneWord Coprocessor15::CompileSendOneWord(bool two, unsigned opc1, CoprocReg CRn, CoprocReg CRm, unsigned opc2) {
        ...
        if (!two && CRn == CoprocReg::C7 && opc1 == 0 && CRm == CoprocReg::C10) {
            switch (opc2) {
                case 4:
                    return Callback{[](void *, std::uint32_t, std::uint32_t) -> std::uint64_t { asm volatile("dsb sy\n\t" : : : "memory"); return 0; }, std::nullopt};
                case 5:
                    return Callback{[](void *, std::uint32_t, std::uint32_t) -> std::uint64_t { asm volatile("dmb sy\n\t" : : : "memory"); return 0; }, std::nullopt};
                default:
                    break;
            }
        }
    ...
    CallbackOrAccessOneWord Coprocessor15::CompileGetOneWord(bool two, unsigned opc1, CoprocReg CRn, CoprocReg CRm, unsigned opc2) {
        if (!two && CRn == CoprocReg::C13 && opc1 == 0 && CRm == CoprocReg::C0) {
            switch (opc2) {
                case 2: return &tpidrurw;
                case 3: return &tpidruro;
                default: break;
            }
        }
    ...
    CallbackOrAccessTwoWords Coprocessor15::CompileGetTwoWords(bool two, unsigned opc, CoprocReg CRm) {
        if (!two && opc == 0 && CRm == CoprocReg::C14)
            return Callback{[](void *, u32, u32) -> u64 { return util::GetTimeTicks(); }, std::nullopt};
