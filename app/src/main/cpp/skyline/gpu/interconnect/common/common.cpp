...
namespace skyline::gpu::interconnect {
    ...
    void ConstantBuffer::Read(CommandExecutor &executor, span<u8> dstBuffer, size_t srcOffset, bool retryGpuReadback, std::source_location location) {
        ContextLock lock{executor.tag, view};
        view.Read(lock.IsFirstUsage(), [retryGpuReadback, location, srcOffset, size = dstBuffer.size()] {
            if (retryGpuReadback)
                throw GpuReadbackRequired{location, srcOffset, size};
