    ...
    void NCE::SvcHandler(u16 svcId, ThreadContext *ctx) {
        ...
        try {
            if (svc) [[likely]] {
                ...
                const u32 getInfoId{static_cast<u32>(svcContext.w1)};
                ...
                const u32 getInfoHandle{traceGetInfo ? static_cast<u32>(svcContext.w2) : 0};
                ...
                if (traceSetHeap) {
                    LOGI("[DL-MEM] SetHeapSize exit: result=0x{:X}, request=0x{:X}, returnedBase=0x{:X}, committed=0x{:X}, heapRegion=0x{:X}",
                         static_cast<u32>(svcContext.w0), setHeapRequest, svcContext.x1, state.process->memory.processHeapSize,
                    ...
                    LOGI("[DL-MEM] GetInfo: id={}, sub={}, handle=0x{:X}, result=0x{:X}, out=0x{:X}, committed=0x{:X}, heapRegion=0x{:X}, systemResource=0x{:X}",
                         getInfoId, getInfoSubId, getInfoHandle, static_cast<u32>(svcContext.w0), svcContext.x1,
