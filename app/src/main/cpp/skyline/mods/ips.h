...
namespace skyline::mods {
    ...
    namespace ips_detail {
        ...
        inline void AddWrite(std::vector<IpsWrite> &writes, u64 offset, const u8 *data, size_t size) {
            ...
            // Atmosphere protects the 0x100-byte NSO header. If a record crosses the
            // boundary, preserve the tail rather than rejecting the entire record.
    ...
    inline std::vector<IpsPatch> CollectIpsPatches(const std::string &publicAppFilesPath, u64 titleId, const std::array<u64, 4> &buildId) {
        ...
        // Match LayeredFS/PCHTXT priority: alphabetically earlier mods have higher
        // priority, so lower-priority patches are applied first and overwritten later.
        std::vector<std::filesystem::path> files;
